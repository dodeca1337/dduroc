...
- **физический прогон миграций**: версия протокола проверяется при подъёме
  ...
  сегментов и применение шагов при чтении ещё не написаны. До первой смены
  версии схемы это не нужно;
- **восстановление активного сегмента при старте**: `SegmentWriter::reopen`
  и `Scan` реализованы и протестированы, но writer при подъёме канала
  всегда создаёт новый сегмент. Оборванный сегмент прошлого запуска
  остаётся незапечатанным — он читается сканом, данные не теряются, но
  его преаллокация не возвращается и разрыв нумерации блоков при чтении
  не проверяется;
...
**Телеметрия** — история изменения значений. **Серия** =
`(тип метрики, тэги)` в пределах неймспейса. Значение — скаляр (температура, давление)
или составное (снимок спектра — массив точек одного измерения).
Сегментно-локальное интернирование: первый сэмпл серии в сегменте пишет
`SeriesDef{локальный id → метрика, origin, тэги}`, далее сэмплы —
`[Δt varint][series_id 1–2B][значение]`, ~6–8 байт на скаляр до сжатия.
Сегмент самоописан: FIFO-ротация не оставляет висячих ссылок на определения.
...
- **Шаги миграции** `vN → vN+1` пишутся разработчиком вместе с изменением
  ...
  ремап id. Цепочка шагов покрывает прыжок через несколько версий.
...
- **Crash-safety**: миграция посегментно — rewrite во временный файл →
  ...
  на следующем старте.
- **Экономия износа**: заголовок сегмента хранит множество id типов событий,
  встречающихся в сегменте. Миграция переписывает только сегменты с
  затронутыми типами; остальные не трогаются (версию хранит мета неймспейса,
  in-place штамп сегмента не нужен). Опционально (позже): ленивая миграция —
  читатель держит декодеры на глубину K версий, перезапись откладывается.
...
dduroc::schema! {
    ...
    migrations { 2 => migrate_v2, 3 => migrate_v3 }
}

...
Совпало несколько префиксов — выигрывает самый длинный (`orc-radio-` уточняет
`orc-`, а не спорит с ним); настройки, заданные при открытии неймспейса, бьют
групповые. Группа задаёт только то, что принадлежит **каждому пишущему каналу

...
- **уборка `epochs.bin`**: `retain_runs` реализован, но не вызывается — файл
  растёт;
- **закрытие каналов по бездействию** и глобальный потолок буферизованных
  байт: при заявленных ~24k неймспейсов упрётся в лимит дескрипторов.
...
- **пределы значений**: диапазон, вне которого значение тревожно, и диапазон,
  вне которого критично.
...
**Пределы значений.** Предел — свойство **установки**, а не измерения: одна и
та же температура нормальна для одного усилителя и аварийна для другого.

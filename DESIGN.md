...
- **физический прогон миграций**: версия протокола проверяется при подъёме
  неймспейса, отсутствие шага в цепочке — ошибка, но переписывание старых
  сегментов и применение шагов при чтении ещё не написаны. Пока их нет,
  `ns-meta` **не штампуется** новой версией: она означает версию последней
  завершённой миграции, и объявить неймспейс мигрированным значило бы
  оставить старые сегменты нечитаемыми навсегда — будущий прогон обошёл бы
  их стороной. Факт незавершённой миграции виден в
...
- **глобальный потолок байт одновременно пишущих каналов** (память, не
  ...
  занятость носителя ограничена суммарным бюджетом хранилища (SPEC §5);
...
- Durability настраиваемая **в рамках одного движка**: часть сообщений
  критичная (устойчивость к потере питания), часть — нет. Класс хранения
  указывается на уровне сообщения.
...
**Бюджеты ротации** — per namespace (у каждого неймспейса свои каналы со
своими бюджетами); дефолты наследуются от группы.
...
- **пределы значений**: диапазон, вне которого значение тревожно (`warn`), и
  диапазон, вне которого аварийно (`alarm`).
...
- **Экономия износа**: footer сегмента хранит множества id типов событий и
  ...
  in-place штамп сегмента не нужен). Затронутые типы объявляются **явно** —
  `1 => migrate_v1 { events: [PowerSet], metrics: [TempPa] }`, — а их
  отсутствие означает «затрагивает всё»: забытый список не должен молча
  превращаться в «не трогаем ничего», иначе шаг обошёл бы стороной ровно ту
  историю, ради которой написан. Экономия ресурса флеша — осознанный opt-in,
  а не умолчание. Опционально (позже): ленивая миграция — читатель держит
  декодеры на глубину K версий, перезапись откладывается.
...
```
ChannelConfig {
    budget_bytes,        // бюджет ротации
    segment_bytes,       // авто: clamp(budget/256, 4 MiB, 256 MiB)
...
Durability per канал:

- `Immediate` — блок закрывается и `fdatasync`'ится сразу при поступлении
  записи (group commit: забирает всё накопившееся). Для критичных данных.
- `Interval(d)` — fdatasync не чаще d (аналог 10-секундного throttle прототипа).
- `Relaxed` — sync только при seal сегмента и shutdown.
...
(Immediate: fdatasync на всплеск), незначимые — в обычный (батчи,
...
- **Отсюда единственный жёсткий предел масштаба**: одновременно пишущих
  каналов не может быть больше, чем `total_budget_bytes / segment_bytes` —
  активный сегмент вытеснить нельзя. При 8 ГиБ потолка и 4 МиБ сегментах это
  ~2000 каналов, пишущих в одну и ту же минуту. Превышение видно в
...
dduroc::schema! {
    ...
    metrics {
        ...
        Spectrum = 0x02 { vtype: blob, store: critical },
    }

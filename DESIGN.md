...
- **миграции спанов и значений отсчётов**: правилам шага доступны события
  ...
  не выразит — у `MigrationOutcome` (исхода шага) нет таких вариантов; понадобится
...
- **Отсюда единственный жёсткий предел масштаба**: одновременно пишущих
  ...
  `Stats::budget_overruns`, а не в кончившемся месте.
...
Границы записываются тем же типом, что и отсчёты (`NumericMetric`), поэтому
...
// корня и подметает временные файлы. Снимок замораживается при открытии,
// и любая недописанность честно доносится как повреждение.

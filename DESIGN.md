...
- **закрытие сегмента по бездействию канала**: буферы канала при
  бездействии уже возвращаются аллокатору (см. SPEC §5), но файловый
  дескриптор активного сегмента остаётся открытым — при заявленных ~24k
  неймспейсов упрётся в лимит дескрипторов. Глобальный потолок байт
  одновременно пишущих каналов — туда же;

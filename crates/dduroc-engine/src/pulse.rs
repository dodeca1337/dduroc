...
//!
//! Отметок две, и различаются они ценой ответа:
//!
//! - **данные** ([`Pulse::data_written`]) — в файл лёг блок. Чтобы его увидеть,
//!   хватает дочитать сегмент, который уже открыт: ни одного `readdir`;
//!   - **устройство** ([`Pulse::shape_changed`]) — появился сегмент, неймспейс
//!   или канал. Тут без обхода каталогов не обойтись, и он стоит на порядок
//!   дороже чтения хвоста.
//!
//! Одной отметкой это не выражается: блоки идут тысячами, каталоги меняются
//! единицами, и общий счётчик заставлял бы перечислять каталоги на каждый
//! блок.
//!
...
pub struct Beat {
    ...
    pub data: u64,
    /// Поколение устройства хранилища: сегменты, каналы, неймспейсы.
    pub shape: u64,
...
impl Pulse {
    ...
    /// Появился сегмент, канал или неймспейс: каталоги надо перечислить снова.

...
pub struct ChannelConfig {
    ...
    ///
    /// Фиксированный, а не производный от бюджета: бюджет общий на класс, а
    /// преаллокацию держит **каждый** пишущий неймспейс — сегмент, выросший
    /// вместе с бюджетом, съел бы весь бюджет одними активными сегментами.
    /// Практический предел одновременно пишущих каналов класса —
    /// `budget_bytes / segment_bytes`.
    ...
    /// внутри корня та же: `<корень>/<неймспейс>/<класс>/`. Смена корня не

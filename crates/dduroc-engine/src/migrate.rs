...
fn rewrite_segment(
    ...
    // Предел роста — по фактическим данным оригинала, и окно растёт по мере
    // надобности. Запас «на всякий случай» тут не бесплатен: временный файл
    // лежит рядом с оригиналом, в бюджете класса не числится и на носитель
    // давит по-настоящему. Прежний двойной запас означал, что прогон требовал
    // втрое больше места, чем занимает сегмент, — на приборе, размеченном под
    // свой бюджет, это ENOSPC на ровном месте. Насколько раздуются записи,
    // знает только цепочка шагов, поэтому единственный честный ответ — расти
    // по факту и отказать ровно тогда, когда места действительно не хватило.
    // Хвост обрежет seal.

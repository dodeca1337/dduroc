...
fn crash_does_not_cost_a_whole_segment_of_budget() {
    ...
    // После аварии сегмент лежит незапечатанным и занимает окно резерва —
    // не весь сегмент: резерв берётся окном, и авария стоит одного окна.
    let crashed = segments(&channel);
    assert_eq!(crashed.len(), 1);
    assert!(
        crashed[0].1 < segment_bytes / 8,
        "авария обошлась в {} байт при сегменте {segment_bytes}: резерв взят \
         не окном, а целиком",
        crashed[0].1
    );

    // Перезапуск обязан вернуть и это окно: незапечатанный сегмент числится в
    // бюджете канала вместе с нерасписанным хвостом, и накопление таких
    // хвостов от аварии к аварии выедало бы бюджет пустотой — ротация
    // принялась бы за живую историю.
    {
        let store =
    ...
    assert!(
        ...
        "сегмент прошлого запуска занимал {} байт, после восстановления — {}: \
         хвост окна не возвращён",
        crashed[0].1,
        after[0].1
    );
    assert!(
        ...
        "после восстановления канал занимает {total} байт при сегменте \
         {segment_bytes}: резерв не возвращён"
    );

...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    //
    // Здесь именно этот случай: хранилище закрыто, `Store` открывать нельзя —
    // он берёт блокировку корня и подметает временные файлы, — поэтому корень
    // и схемы называются руками. Своё живое хранилище читается им самим:
    // `store.reader()?` знает и корни (включая вынесенные носители классов), и
    // схемы поднятых неймспейсов. Пример — в `04_operations`.
    // -----------------------------------------------------------------------

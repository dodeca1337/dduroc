//! Значения телеметрии.
//!
//! Тип значения фиксируется в определении серии ([`SeriesDef`]) и **дублируется
//! в младших битах флагов записи** [`Sample`]: без этого сэмпл нельзя пропустить,
//! не зная его серию, и одна повреждённая `SeriesDef` делала бы нечитаемым
//! весь остаток блока. Дублирование бесплатно — флаги всё равно есть в первом
//! байте записи.
//!
//! [`SeriesDef`]: crate::record::SeriesDef
